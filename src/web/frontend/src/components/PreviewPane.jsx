...
const CompactTemplate = ({ data }) => {
    ...
    const visibleData = React.useMemo(() => ({
        ...data,
        work: data.work?.filter(i => i.visible !== false) || [],
        education: data.education?.filter(i => i.visible !== false) || [],
        projects: data.projects?.filter(i => i.visible !== false) || [],
        skills: data.skills?.filter(i => i.visible !== false) || []
    }), [data]);
    ...
    return (
        <div className="font-serif text-black p-[10mm]">
            ...
            <div className="mb-4">
                ...
                {visibleData.education?.map((edu, i) => (
            ...
            <div>
                ...
                <div className="text-sm">
                    ...
                    {visibleData.skills?.map(s => typeof s === 'object' ? (s.keywords?.join(', ') || s.name) : s).join(', ')}
                </div>
            </div>
        </div>
    );
...
const ModernTemplate = ({ data }) => {
    ...
    const flatSkills = React.useMemo(() => {
        if (!visibleData.skills) return [];
        return visibleData.skills.flatMap(s => {
        ...
        <div className="font-sans text-gray-800 p-[15mm]">
            ...
            <div className="grid grid-cols-[2fr_1fr] gap-10">
                <main>
                    ...
                    {
                        visibleData.education && visibleData.education.length > 0 && (
                            <section className="mb-8">
                                <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-4 flex items-center gap-2">
    ...
    return (
        <div className="bg-zinc-900 h-full flex flex-col border-l border-zinc-800 relative print:border-none">
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-8 flex justify-center bg-zinc-900 scrollbar-hide print:p-0 print:overflow-visible">
                {/* 
                    Print Layout: 
                    - Removed fixed/h-screen to allow multi-page flow.
                    - Using relative/static to let it flow naturally.
                    - Parents have print:hidden (Sidebar/Editor) so this should be the only thing visible.
                */}
                <div className="print:static print:w-full print:h-auto print:block">
                    <div className="bg-white shadow-2xl transition-all duration-300 print:shadow-none relative print:w-full"
                        ...
                        <div className="absolute top-[297mm] left-0 w-full border-b border-red-400 border-dashed opacity-50 pointer-events-none print:hidden flex items-end justify-end pb-1 pr-2">
                            <span className="text-[10px] text-red-400 font-mono uppercase">Page 1 Limit</span>
                        </div>
                        {/* Optional 2nd Page Guide? */}
                        <div className="absolute top-[594mm] left-0 w-full border-b border-red-400 border-dashed opacity-50 pointer-events-none print:hidden flex items-end justify-end pb-1 pr-2">
                            <span className="text-[10px] text-red-400 font-mono uppercase">Page 2 Limit</span>
                        </div>
